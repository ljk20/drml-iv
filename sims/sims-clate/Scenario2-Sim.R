...
Z <- rbinom(n, 1, plogis(0.4*X[,1] - 0.8*X[,2]))

## enforce monotonicity so CLATE = CATE = r.X
p0 <- plogis(-0.3 - 0.4*X[,1] - 0.14*X[,2] + 0.7 * U)
p1 <- plogis(-0.3 - 0.4*X[,1] - 0.14*X[,2] + 1.1 + 0.7 * U)
A0 <- rbinom(n, 1, p0)
A1 <- A0 + (1-A0)*rbinom(n, 1, (p1 - p0) / (1 - p0))
A <- (1 - Z)*A0 + Z*A1

# CATE function
r.X <- (ATE - 4 * X[,1] + 6 * (X[,2] - 0.3))
ite.true <- quantile(r.X, probs = seq(.1, .9, by = .1))
clate.true <- (ATE - 4 * .5 + 6 * (1 - 0.3))
...
eval_data = function(dat, ite.true, clate.true){
  # TSLS
  out.ivreg <- ivreg(Y ~ A + X1 + X2 | Z + X1 + X2, data=dat)
  TSLS = tibble( est = out.ivreg$coef[2])
...
save(scenarios, file="./output/scenario2_results.RData")
